//! VPN-to-mesh packet routing bridge.
//!
//! Converts outbound packets from the local VPN path into mesh sends (via
//! callback) and validates/injects inbound mesh packets back into the local
//! VPN interface.
...
use super::vpn_service::{parse_source_ip, PacketHandler, VpnService};
use crate::core::error::{MeshInfinityError, Result};
use crate::core::{PeerId, PeerInfo};

type SendCallback = dyn Fn(PeerId, Vec<u8>) -> Result<()> + Send + Sync;
...
pub struct MeshPacketRouter {
    ...
    send_callback: Arc<RwLock<Option<Box<SendCallback>>>>,
...
impl MeshPacketRouter {
    /// Create packet router bound to a VPN service and local tunnel IP.
    pub fn new(vpn_service: Arc<VpnService>, local_ip: IpAddr) -> Self {
        Self {
            vpn_service,
    ...
    fn route_outbound(&self, dest_ip: IpAddr, packet: Vec<u8>) -> Result<()> {
        ...
        let peer_id = self.vpn_service.get_ip_peer(&dest_ip).ok_or_else(|| {
            MeshInfinityError::NetworkError(format!("No peer found for IP {}", dest_ip))
        })?;
            ...
            if let Some(actual_ip) = parse_source_ip(packet) {
                if actual_ip != expected_ip {
                    return Err(MeshInfinityError::SecurityError(format!(
                        "IP spoofing detected: expected {}, got {}",
...
impl PacketHandler for MeshPacketRouter {
    /// Handle outbound VPN packet by routing it toward destination peer.
    fn handle_outbound(&mut self, dest_ip: IpAddr, packet: Vec<u8>) -> Result<()> {
        self.route_outbound(dest_ip, packet)
    }

    /// Inbound path is handled by [`MeshPacketRouter::handle_inbound_from_mesh`].
...
mod tests {
    ...
    /// Basic construction sanity-check for router-local addressing.
    #[test]
