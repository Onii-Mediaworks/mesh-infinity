...
use super::mesh_address::{ConversationId, ConversationTuple, DeviceAddress, MeshAddress};
use crate::core::error::{MeshInfinityError, Result};
use crate::core::{PeerId, Protocol, TransportType, TrustLevel};
...
pub enum TrafficMatcher {
    ...
    DestinationIp { ip: IpAddr, prefix_len: Option<u8> },
    ...
    MeshPrefix { device_prefix: Vec<u8> },
...
pub enum RoutingAction {
    ...
    DirectPeer { destination: DeviceAddress },
...
impl PolicyRouter {
    ...
    pub fn register_exit_node(&self, info: ExitNodeInfo) {
        ...
        if let Some(existing) = nodes
            .iter_mut()
            .find(|n| n.device_address == info.device_address)
        {
            *existing = info;
        } else {
            nodes.push(info);
    ...
    fn matches(&self, matcher: &TrafficMatcher, packet: &PacketInfo) -> bool {
        match matcher {
            ...
            TrafficMatcher::DestinationPort(port) => packet.dest_port == Some(*port),
            ...
            TrafficMatcher::Protocol(proto) => packet.protocol == *proto,
            ...
            TrafficMatcher::All(matchers) => matchers.iter().all(|m| self.matches(m, packet)),

            TrafficMatcher::AnyOf(matchers) => matchers.iter().any(|m| self.matches(m, packet)),

            TrafficMatcher::Not(inner) => !self.matches(inner, packet),
        }
    }
    ...
    fn build_decision(
        ...
        let destination_addr = match &action {
            RoutingAction::DirectPeer { destination } => destination.with_conversation(conv_id),
    ...
    fn record_conversation(&self, decision: &RoutingDecision) {
        let tuple = ConversationTuple::new(decision.source_addr, decision.destination_addr);
    ...
    fn select_best_exit_node(&self) -> Result<ExitNodeInfo> {
        ...
        best.cloned().ok_or_else(|| {
            MeshInfinityError::NetworkError("No suitable exit node found".to_string())
        })
    }
    ...
    pub fn cleanup_expired(&self) {
        ...
        conversations
            .retain(|_, conv| now.duration_since(conv.last_activity) < self.conversation_timeout);

        // Also clean up expired rules
        let mut rules = self.rules.write().unwrap();
        rules.retain(|rule| rule.expires_at.map(|exp| now < exp).unwrap_or(true));
    }
...
impl Default for PolicyRouter {
    /// Create default policy router with empty rule/state tables.
    fn default() -> Self {
        Self::new()
    }
}
...
impl PacketInfo {
    ...
    /// Parse minimal IPv4 packet metadata used by policy matchers.
    ...
    /// Parse minimal IPv6 packet metadata used by policy matchers.
...
impl Protocol {
    /// Convert protocol enum to canonical IP protocol number.
    fn to_u8(&self) -> u8 {
        match self {
            Protocol::TCP => 6,
            Protocol::UDP => 17,
...
impl RoutingRule {
    ...
    pub fn peer_direct(peer_addr: DeviceAddress) -> Self {
        ...
        let id_suffix: String = bytes[..8].iter().map(|b| format!("{:02x}", b)).collect();
        Self {
    ...
    pub fn block_ip(ip: IpAddr, reason: &str) -> Self {
        Self {
            ...
            matcher: TrafficMatcher::DestinationIp {
                ip,
                prefix_len: None,
            },
...
mod tests {
    ...
    /// `Any` matcher should accept arbitrary packet metadata.
    #[test]
    fn test_matcher_any() {
    ...
    /// Destination-port matcher should match only configured port.
    #[test]
    ...
    /// Compound matcher should require all constituent conditions to pass.
    #[test]
    ...
    /// Domain matcher should support exact and wildcard patterns.
    #[test]
    ...
    /// Prefix matcher should honor CIDR mask boundaries.
    #[test]
