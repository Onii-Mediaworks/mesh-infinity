//! Payload obfuscation and packet-shaping helpers.
//!
//! Provides lightweight transforms used to reduce protocol fingerprinting risk
//! on active mesh traffic. These transforms are *not* replacements for primary
//! encryption; they are an additional camouflage layer.
...
impl TrafficShaper {
    /// Create traffic shaper with shaping enabled/disabled.
    ...
    pub fn shape_packet(&self, packet: &[u8]) -> Result<Vec<u8>> {
        ...
        let target_size = shaped.len().div_ceil(512) * 512;
...
mod tests {
    ...
    /// XOR transform must round-trip with the same key.
    #[test]
    ...
    /// `None` mode must leave payload bytes untouched.
    #[test]
    ...
    /// XOR mode must obfuscate and then recover original payload.
    #[test]
    ...
    /// Shaper should pad to expected boundary and unshape losslessly.
    #[test]
