...
impl MeshAddress {
    ...
    pub fn is_loopback(&self) -> bool {
        self.0[..TOTAL_ADDRESS_SIZE - 1].iter().all(|&b| b == 0) && self.0[TOTAL_ADDRESS_SIZE - 1] == 1
    }
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        if parts.len() != 8 {
            return Err(MeshInfinityError::NetworkError(
                format!("Invalid mesh address: expected 8 groups, got {}", parts.len())
            ));
        }
        ...
        for (i, part) in parts.iter().enumerate() {
            if part.len() != 8 {
                return Err(MeshInfinityError::NetworkError(
                    format!("Invalid group {}: expected 8 hex chars, got {}", i, part.len())
                ));
            }

            let group_value = u32::from_str_radix(part, 16)
                .map_err(|_| MeshInfinityError::NetworkError(
                    format!("Invalid hex in group {}: {}", i, part)
                ))?;
...
impl FromStr for DeviceAddress {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        if parts.len() != 5 {
            return Err(MeshInfinityError::NetworkError(
                format!("Invalid device address: expected 5 groups, got {}", parts.len())
            ));
        }
        ...
        for (i, part) in parts.iter().enumerate() {
            if part.len() != 8 {
                return Err(MeshInfinityError::NetworkError(
                    format!("Invalid group {}: expected 8 hex chars", i)
                ));
            }

            let group_value = u32::from_str_radix(part, 16)
                .map_err(|_| MeshInfinityError::NetworkError(
                    format!("Invalid hex in group {}", i)
                ))?;
...
impl FromStr for ConversationId {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        if parts.len() != 3 {
            return Err(MeshInfinityError::NetworkError(
                format!("Invalid conversation ID: expected 3 groups, got {}", parts.len())
            ));
        }
...
mod tests {
    ...
    fn test_address_display() {
        ...
        assert_eq!(addr.to_string(), "00000000:00000000:00000000:00000000:00000000:00000000:00000000:00000000");

        let addr = MeshAddress::from_groups([0x12345678, 0x9abcdef0, 0x11111111, 0x22222222,
                                              0x33333333, 0x44444444, 0x55555555, 0x66666666]);
        assert_eq!(addr.to_string(), "12345678:9abcdef0:11111111:22222222:33333333:44444444:55555555:66666666");
    }
    ...
    fn test_device_conversation_split() {
        let addr = MeshAddress::from_groups([0x11111111, 0x22222222, 0x33333333, 0x44444444,
                                              0x55555555, 0xaaaaaaaa, 0xbbbbbbbb, 0xcccccccc]);
