...
use crate::core::{PeerId, TrustLevel, TransportType};
use crate::core::error::{MeshInfinityError, Result};
use super::mesh_address::{MeshAddress, DeviceAddress, ConversationId};
...
impl HopRouter {
    ...
    pub fn new(our_address: DeviceAddress, our_peer_id: PeerId) -> Self {
        Self {
            our_address,
            our_peer_id,
            ...
            pending_announcements: Arc::new(RwLock::new(VecDeque::new())),
            config: HopRouterConfig::default(),
        }
    }

    /// Create with custom configuration
    pub fn with_config(our_address: DeviceAddress, our_peer_id: PeerId, config: HopRouterConfig) -> Self {
        Self {
            our_address,
            our_peer_id,
            neighbors: Arc::new(RwLock::new(HashMap::new())),
    ...
    pub fn process_announcement(&self, from_peer: PeerId, announcement: ReachabilityAnnouncement) -> bool {
        ...
        let our_latency = Duration::from_millis(announcement.latency_estimate_ms as u64)
            + neighbor.latency;
    ...
    pub fn generate_announcements(&self) -> Vec<ReachabilityAnnouncement> {
        ...
        announcements.push(ReachabilityAnnouncement {
            ...
            sequence: current_time_ms(), // Use time as sequence
        });

        // Announce all destinations we can reach
        for (dest, routes) in table.iter() {
            // Use the best route for announcement
    ...
    pub fn route_packet(
        ...
        // Check if we already have an active link for this conversation
        {
            ...
            if let Some(link) = links.get(&conv_id) {
                ...
                let neighbors = self.neighbors.read().unwrap();
                if neighbors.get(&link.next_hop).map(|n| n.is_active).unwrap_or(false) {
        ...
        let decision = self.next_hop_for(&dest_device)
            .ok_or_else(|| MeshInfinityError::NetworkError(
                format!("No route to destination {:?}", dest_device)
            ))?;
    ...
    pub fn cleanup(&self) {
        ...
        // Clean inactive links
        {
            ...
            links.retain(|_, link| now.duration_since(link.last_activity) < self.config.link_timeout);
        }
...
mod tests {
    ...
    fn test_process_announcement() {
        ...
        assert!(decision.is_some());
        assert_eq!(decision.unwrap().next_hop, make_peer_id(2));
        assert_eq!(decision.unwrap().hop_count, 2);
    }
