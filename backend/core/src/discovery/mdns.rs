...
impl MdnsDiscovery {
    ...
    pub fn new(local_peer_id: PeerId, local_port: u16) -> Result<Self> {
        let daemon = ServiceDaemon::new()
            .map_err(|e| MeshInfinityError::NetworkError(format!("Failed to create mDNS daemon: {}", e)))?;
    ...
    pub fn start(&self) -> Result<()> {
        // Set running flag
        {
            ...
            if *running {
                return Err(MeshInfinityError::InvalidInput("mDNS discovery already running".to_string()));
            }
            *running = true;
        }
    ...
    pub fn stop(&self) -> Result<()> {
        ...
        *running = false;
        ...
        self.daemon.shutdown()
            .map_err(|e| MeshInfinityError::NetworkError(format!("Failed to shutdown mDNS: {}", e)))?;
    ...
    pub fn discovered_peers(&self) -> Result<Vec<PeerInfo>> {
        ...
        let peers = self.discovered_peers.read()
            .map_err(|e| MeshInfinityError::LockError(format!("Discovered peers lock poisoned: {}", e)))?;
    ...
    fn register_service(&self) -> Result<()> {
        ...
        let service_info = ServiceInfo::new(
            ...
            self.local_port,
            Some(properties),
        ).map_err(|e| MeshInfinityError::NetworkError(format!("Failed to create service info: {}", e)))?;

        self.daemon.register(service_info)
            .map_err(|e| MeshInfinityError::NetworkError(format!("Failed to register mDNS service: {}", e)))?;
    ...
    fn browse_services(&self) -> Result<()> {
        let receiver = self.daemon.browse(SERVICE_TYPE)
            .map_err(|e| MeshInfinityError::NetworkError(format!("Failed to browse mDNS: {}", e)))?;
            ...
            while let Ok(event) = receiver.recv() {
                ...
                match event {
                    ServiceEvent::ServiceResolved(info) => {
                        if let Err(e) = Self::handle_service_resolved(info, &discovered_peers, &local_peer_id) {
    ...
    fn handle_service_resolved(
        ...
        let peer_id_str = properties.get("peer_id")
            .map(|v| v.val_str())
            .ok_or_else(|| MeshInfinityError::InvalidInput("No peer_id in mDNS service".to_string()))?;
        ...
        if addresses.is_empty() {
            return Err(MeshInfinityError::InvalidInput("No addresses in mDNS service".to_string()));
        }
        ...
        peers.insert(peer_id, DiscoveredPeer {
            peer_info,
    ...
    fn handle_service_removed(
        ...
        let instance_name = full_name.split('.').next()
...
fn hex_to_peer_id(hex: &str) -> Result<PeerId> {
    if hex.len() != 64 {
        return Err(MeshInfinityError::InvalidInput(
            format!("Invalid peer ID length: expected 64 hex chars, got {}", hex.len())
        ));
    }
