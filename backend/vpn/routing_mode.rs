...
pub struct VpnConfig {
    ...
    /// Selected Tailscale exit node by tailnet peer name.
...
impl VpnManager {
    ...
    pub fn set_mode(
        ...
        // ExitNode requires either a mesh exit peer or a Tailscale exit node.
        if mode == RoutingMode::ExitNode
            && self.config.exit_peer_id.is_none()
            && self.config.tailscale_exit_node.is_none()
        {
...
mod tests {
    ...
    fn test_kill_switch_strict() {
        let mut mgr = VpnManager::new();
        mgr.config.exit_peer_id = Some([0x01; 32]);
        mgr.set_mode(RoutingMode::ExitNode, ThreatContext::Normal, false)
            .unwrap();
