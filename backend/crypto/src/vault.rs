...
impl Vault {
    ...
    pub fn unwrap_with_password(slot: &WrappedKeySlot, password: &str) -> Result<VaultKey, VaultError> {
    ...
    pub fn wrap_with_mnemonic(key: &VaultKey, mnemonic: &Mnemonic) -> Result<WrappedKeySlot, VaultError> {
        let salt = random_bytes(16);
    ...
    pub fn unwrap_with_mnemonic(slot: &WrappedKeySlot, mnemonic: &Mnemonic) -> Result<VaultKey, VaultError> {
        let salt = decode_b64(&slot.salt_b64)?;
        let derived = derive_recovery_key(mnemonic, &salt)?;
        decrypt_key_slot(&derived, slot)
    }
    ...
    pub fn decrypt_bytes(key: &VaultKey, payload: &EncryptedPayload) -> Result<Vec<u8>, VaultError> {
    ...
    pub fn encrypt_file<P: AsRef<Path>>(key: &VaultKey, input: P, output: P) -> Result<FileHeader, VaultError> {
    ...
    pub fn decrypt_file<P: AsRef<Path>>(key: &VaultKey, input: P, output: P) -> Result<(), VaultError> {
        let mut input = File::open(input)?;
        let mut output = File::create(output)?;
...
fn derive_password_key(password: &str, salt: &[u8], params: &KdfParams) -> Result<[u8; 32], VaultError> {
