...
impl MessageCrypto {
    ...
    pub fn generate() -> Result<Self> {
        ...
        getrandom::getrandom(&mut dh_secret_bytes)
            .map_err(|_| MeshInfinityError::CryptoError("Failed to generate random bytes".into()))?;
    ...
    pub fn decrypt_message(
        ...
        let double_signed = self.aead_decrypt(
            &*final_key,
            &ciphertext[X25519_PUBLIC_KEY_SIZE..],
        )?;

        // Step 3 (reverse): Verify outer signature
        if double_signed.len() < SIGNATURE_SIZE {
            return Err(MeshInfinityError::CryptoError("Missing outer signature".into()));
        }
        ...
        let outer_signature = Signature::new(outer_sig_array);

        sender_public
            .verify(trust_encrypted, &outer_signature)
            .map_err(|_| MeshInfinityError::CryptoError("Outer signature verification failed".into()))?;
        ...
        if signed_message.len() < SIGNATURE_SIZE {
            return Err(MeshInfinityError::CryptoError("Missing inner signature".into()));
        }
        ...
        let inner_signature = Signature::new(inner_sig_array);

        sender_public
            .verify(message, &inner_signature)
            .map_err(|_| MeshInfinityError::CryptoError("Inner signature verification failed".into()))?;
    ...
    fn aead_decrypt(&self, key: &[u8; 32], ciphertext: &[u8]) -> Result<Vec<u8>> {
        if ciphertext.len() < NONCE_SIZE + TAG_SIZE {
            return Err(MeshInfinityError::CryptoError("Ciphertext too short".into()));
        }
...
impl MessageHeader {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        if data.len() < X25519_PUBLIC_KEY_SIZE {
            return Err(MeshInfinityError::CryptoError("Message too short for header".into()));
        }
