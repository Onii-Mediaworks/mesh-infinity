...
impl DeniableAuth {
    ...
    pub fn create_ring_signature(
        ...
        let ephemeral = self.ephemeral_keys.get(session_id).ok_or_else(|| {
            MeshInfinityError::CryptoError("No ephemeral key for session".to_string())
        })?;
    ...
    fn lsag_sign(
        ...
        for (i, r_value) in r_values.iter_mut().enumerate().take(n) {
            if i != signer_idx {
                rng.fill_bytes(r_value);
            }
        }
        ...
        // Bind the ring proof material to an Ed25519 signature.
        //
        // Verification checks that at least one ring member can validate this
        // binding signature over the canonical transcript, preserving anonymity
    ...
    /// Build canonical transcript bytes used for ring binding signatures.
...
impl RingSignature {
    ...
    pub fn verify(&self, message: &[u8]) -> Result<bool> {
        ...
        // Ring must close properly.
        if computed_c[0] != c_values[0] {
            return Ok(false);
        }

        // Binding signature must validate for at least one ring member.
        let transcript =
            DeniableAuth::signature_transcript(message, key_image, &self.ring, c_values, r_values);

        Ok(self
            .ring
            .iter()
            .any(|pk| pk.verify(&transcript, &self.signature).is_ok()))
    }
